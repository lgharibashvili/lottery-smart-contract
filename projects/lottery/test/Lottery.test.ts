...
const PRICE_MATIC = 1.2;

function gasToMATIC(gas: number, gwei: number = 5) {
  const num = gas * gwei * 10 ** -9;
  return num.toFixed(4);
}

function gasToUSD(gas: number, gwei: number = 5, priceMATIC: number = PRICE_MATIC) {
  const num = gas * priceMATIC * gwei * 10 ** -9;
...
contract("Lottery V2", ([alice, bob, carol, david, erin, operator, treasury, injector]) => {
  ...
  let _priceTicket = parseEther("0.5");
  ...
  // Contracts
  let lottery, mockUSDT, randomNumberGenerator;
  ...
  before(async () => {
    // Deploy MockUSDT
    mockUSDT = await MockERC20.new("Mock USDT", "USDT", _totalInitSupply);

    // Deploy MockRandomNumberGenerator
    randomNumberGenerator = await MockRandomNumberGenerator.new({ from: alice });

    // Deploy Lottery
    lottery = await Lottery.new(mockUSDT.address, randomNumberGenerator.address, { from: alice });
  ...
  describe("LOTTERY #1 - CUSTOM RANDOMNESS", async () => {
    ...
    it("Users mint and approve USDT to be used in the lottery", async () => {
      for (let thisUser of [alice, bob, carol, david, erin, injector]) {
        await mockUSDT.mintTokens(parseEther("100000"), { from: thisUser });
        await mockUSDT.approve(lottery.address, parseEther("100000"), {
          from: thisUser,
        });
      }
    });
    ...
    it("Operator starts lottery", async () => {
      ...
      console.info(
        `        --> Cost to start the lottery: ${gasToMATIC(result.receipt.gasUsed)} (USD: ${gasToUSD(
    ...
    it("Bob buys 100 tickets", async () => {
      ...
      console.info(
        `        --> Cost to buy the first 100 tickets: ${gasToMATIC(result.receipt.gasUsed)} (USD: ${gasToUSD(
          result.receipt.gasUsed
        )})`
      );

      expectEvent.inTransaction(result.receipt.transactionHash, mockUSDT, "Transfer", {
        from: bob,
        to: lottery.address,
    ...
    it("Carol buys 1 ticket", async () => {
      ...
      console.info(
        `        --> Cost to buy a stand-alone ticket: ${gasToMATIC(result.receipt.gasUsed)} (USD: ${gasToUSD(
          result.receipt.gasUsed
        )})`
      );
    ...
    it("David buys 10 tickets", async () => {
      ...
      console.info(
        `        --> Cost to buy 10 tickets: ${gasToMATIC(result.receipt.gasUsed)} (USD: ${gasToUSD(
          result.receipt.gasUsed
        )})`
      );
    ...
    it("Owner does 10k USDT injection", async () => {
      ...
      console.info(
        `        --> Cost to do injection: ${gasToMATIC(result.receipt.gasUsed)} (USD: ${gasToUSD(
          result.receipt.gasUsed
        )})`
      );
    ...
    it("Operator closes lottery", async () => {
      ...
      console.info(
        `        --> Cost to close lottery: ${gasToMATIC(result.receipt.gasUsed)} (USD: ${gasToUSD(
          result.receipt.gasUsed
        )})`
      );
    });
    ...
    it("Numbers are drawn (9/9/9/9/9/9)", async () => {
      ...
      console.info(
        `        --> Cost to draw numbers (w/o ChainLink): ${gasToMATIC(result.receipt.gasUsed)} (USD: ${gasToUSD(
          result.receipt.gasUsed
        )})`
      );
    });
    ...
    it("David claims the jackpot", async () => {
      // 10,000 + 47.525 + 0.5 + 4.9775 = 10053.0025 USDT collected
      // 10053.0025 * (1-0.20) * 0.5 = 4021.201 USDT
      ...
      console.info(
        `        --> Cost to claim ticket: ${gasToMATIC(result.receipt.gasUsed)} (USD: ${gasToUSD(
          result.receipt.gasUsed
        )})`
      );
    ...
    it("Bob claims 10 winning tickets he bought", async () => {
      // 10,000 + 47.525 + 0.5 + 4.9775 = 10053.0025 USDT collected
      // 10053.0025 * (1-0.20) * 0.02 = 160.84804 USDT
      // 10053.0025 * (1-0.20) * 0.03 = 241.27206 USDT
      ...
      console.info(
        `        --> Cost to claim 9 tickets: ${gasToMATIC(result.receipt.gasUsed)} (USD: ${gasToUSD(
          result.receipt.gasUsed
        )})`
      );
    ...
    describe("Role exceptions", async () => {
      it("Owner can recover funds only if not USDT token", async () => {
        // Deploy Random Token
        ...
        await expectRevert(
          lottery.recoverWrongTokens(mockUSDT.address, parseEther("1"), { from: alice }),
          "Cannot be lottery currency"
        );
      });
      ...
      it("Only owner/injector can call owner functions", async () => {
        ...
        await expectRevert(
          lottery.recoverWrongTokens(mockUSDT.address, parseEther("10"), { from: operator }),
          "Ownable: caller is not the owner"
        );
